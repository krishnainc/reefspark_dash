...
def process_temperature_to_parquet(start_year, end_year):
        ...
        try:
            ...
            with fsspec.open(raw_file_path, **storage_options) as f:
                ...
                for i, row_count in enumerate(temp_rows):
                    ...
                    temps = temp_values[cursor:cursor + row_count]
                    depths = z_values[cursor:cursor + row_count]
                    cursor += row_count

                    if len(temps) == 0 or len(depths) == 0:
                        continue

                    for temp, z in zip(temps, depths):
                        if pd.isna(temp) or pd.isna(z):
                            continue
                        rows.append({
                            'time': time[i],
                            'lat': lat[i],
                ...
                if not rows:
                    print(f"  -> No valid temperature data found for {year}. Skipping.")
                    continue
...
process_temperature_to_parquet(1800, 2025)