...
def process_oxygen_to_parquet(start_year, end_year):
        ...
        try:
            ...
            with fsspec.open(raw_file_path, **storage_options) as f:
                ...
                for i, row_count in enumerate(oxy_rows):
                    ...
                    if len(oxys) == 0 or len(depths) == 0:
                        continue

                    for oxy, z in zip(oxys, depths):
                        if pd.isna(oxy) or pd.isna(z):
                            continue
                        rows.append({
                            'time': time[i],
                            'lat': lat[i],
                ...
                if not rows:
                    print(f"  -> No valid oxygen data found for {year}. Skipping.")
                    continue
...
process_oxygen_to_parquet(1800, 2025)